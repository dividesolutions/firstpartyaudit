...
//   signals: {
//     watchedSubdomainRootCookieSets: SubdomainCookieSetSignal[];
//   }; // overall routing signal
...
// type CaptureBundle = {
//   loadMs: number;
//   setCookieHeaders: string[]; // you can keep this if you want
//   cookieSetEvents: Array<{
//     responseUrl: string;
//     responseHostname: string;
//     status: number;
//     setCookie: string; // a single Set-Cookie header line
//   }>;
//   requests: Array<{
//     url: string;
//     method: string;
...
//         // keep old behavior if you still want it
//         setCookieHeaders.push(line);

//         // NEW: preserve which response set it
...
//     // keep your existing "attach response info to request" block
...
//   return { loadMs, setCookieHeaders, cookieSetEvents, requests: reqs };
// }
...
//   if (h === rd) return null;
//   if (!h.endsWith(`.${rd}`)) return null;

//   const remainder = h.slice(0, h.length - rd.length - 1); // remove ".rootdomain"
//   const label = remainder.split(".")[0]; // left-most label
//   return label || null;
// }
...
//   cookieName: string;
//   cookieDomain: string; // Domain= from Set-Cookie
//   path?: string;
//   secure: boolean;
//   httpOnly: boolean;
//   maxAge?: number | null;
...
//     // We care about cookies explicitly set for root domain
//     const cookieDomain =
//       normalizeCookieDomain(parsed.domain) ?? ev.responseHostname.toLowerCase();

//     function getSubdomainLabels(
//       hostname: string,
//       rootDomain: string,
//     ): string[] {
//       const h = hostname.toLowerCase();
...
function scoreCookiePlatformPresence(platformsPresent: string[]): number {
  const n = platformsPresent.length; // 0..4
  return Math.round((n / 4) * 90);
}
...
function buildRecommendedActions(input: {
  ...
  if (cookiePlatformsPresent.length === 0) {
    return [
      ...
      "If you expect Meta/GA/Ads/TikTok cookies, verify tags fire and cookies are not blocked by consent or browser policies.",
    ].join(" ");
  }
